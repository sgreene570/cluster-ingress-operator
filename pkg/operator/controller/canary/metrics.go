...
var (
	...
	// This metric is tentative as there are lots of potential reasons for the canary check to fail
	CanaryRouteError = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "ingress_canary_route_check_error",
			Help: "Counters for individual canary check errors",
		}, []string{"host", "error"})

	// Populate prometheus collector.
	// Individual metrics are saved as public variables.
...
// SetCanaryRouteReachable is a wrapper function to
// mark the canary route as online and available.
func SetCanaryRouteReachable(host string) {
	CanaryRouteReachable.WithLabelValues(host).Set(1)
}

// SetCanaryRouteUnreachable is a wrapper function to
// mark the canary route as offline and unavailable.
func SetCanaryRouteUnreachable(host string) {
...
func StartMetricsListener(addr string, stopCh chan struct{}) {
	// These get registered in controller-runtime's registry via an init in the internal/controller/metrics package.
	// Unregister the controller-runtime metrics, so that we can combine the controller-runtime metric's registry
	// with that of the ingress-operator. This is a supported flow and shouldn't have any side effects, as long
	// as no 2 metrics share the same name.
	...
	// with the controller runtimes metrics registry.
